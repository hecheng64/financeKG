...
CP="${sDir}/lib/commons-csv-1.6.jar:${sDir}/conf:${JANUSGRAPH_HOME}/lib/*"
class=$1
case $class in
    gencsv)
        shift
        java -cp "$CP":"${utilityJar}" com.dataIn.janusgraph.utils.generator.JanusGraphBench "$@"
        ;;
    import)
        shift
        java -cp "$CP":"${utilityJar}" com.dataIn.janusgraph.utils.importer.BatchImport "$@"
        ;;
    loadsch)
        shift
        java -cp "$CP":"${utilityJar}" com.dataIn.janusgraph.utils.importer.schema.SchemaLoader "$@"
        ;;
    *)
